...
public class Frm_SuaPhuongTien extends JFrame implements ActionListener, MouseListener {
	...
	private Frm_PhuongTien frm_PT;
	public Frm_SuaPhuongTien(JPanel panel, boolean modal) {
		
		setTitle("Sửa thông tin phương tiện");
		setSize(1000, 550);
		setLocationRelativeTo(null);
		setResizable(false);
		
		frm_PT = new Frm_PhuongTien();
		frm_PT =(Frm_PhuongTien) panel;
		add(lblTitle = new JLabel("Sửa thông tin phương tiện", SwingConstants.CENTER), BorderLayout.NORTH);
		...
		txtMaPhuongTien.setEditable(false);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource().equals(btnLuu)) {
			if(checkInput()) {
				...
				frmPhuongTien.suaPhuongTien(s);
			
			}
			this.dispose();
		}else if (e.getSource().equals(btnDong)) {
			this.dispose();
		}
...
//			return false;
//		}if(!(loaiPT.length()>0 && loaiPT.matches("[a-zA-Z ]+"))) {
...
//		}
	...
	public void setEditData(PhuongTien phuongTien) {
		// TODO Auto-generated method stub
		txtMaPhuongTien.setText(phuongTien.getMaPhuongTien());
		txtLoaiPhuongTien.setText(phuongTien.getLoaiPhuongTien());
		txtBienSo.setText(phuongTien.getBienSoXe());
		txtGhiChu.setText(phuongTien.getGhiChu());
