...
public class NhanVien_DAO {
	...
	public NhanVien searchNhanVienTheoMa(String maNhanVien) {
		...
		try {
			...
			String sql = "Select " + " NV.maNhanVien, hoTenNhanVien, gioiTinh, ngaySinh, soDienThoai\r\n"
			...
			while (rs.next()) {
				...
				nv.setTenNhanVien(rs.getString("hoTenNhanVien"));
	...
	public void updateNhanVien(NhanVien nhanVienNew) {
		...
		try {
			ConnectDB.getInstance().connect();
			Connection con = ConnectDB.getConnection();
			String sql = "update NhanVien set hoTenNhanVien = ?, gioiTinh = ?, ngaySinh = ?, soDienThoai = ?  where maNhanVien = ?";
			ps = con.prepareStatement(sql);
			ps.setString(1, nhanVienNew.getTenNhanVien());
			ps.setBoolean(2, nhanVienNew.isGioiTinh());
			ps.setDate(3, nhanVienNew.getNgaySinh());
			ps.setString(4, nhanVienNew.getSoDienThoai());
